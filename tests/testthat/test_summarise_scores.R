...
out <- compare_timeseries(rts, cases, models,
...
exp_scores <- setdiff(
  names(out$rt_scores),
  c(
    "timeseries", "model", "forecast_date", "date",
    "horizon"
  )
)
...
test_that("Default outputs have proper lengths and names", {
  ...
  expect_named(summary_out, c(
    "score", "model", "bottom", "lower", "median",
    "mean", "upper", "top", "sd"
  ))
...
test_that("Summary over variables has proper lengths and names", {
  ...
  expect_equal(
    nrow(summary_horizon),
    length(models) * length(exp_scores) * max(out$rt_scores$horizon)
  )

  expect_named(summary_horizon, c(
    "horizon", "score", "model", "bottom", "lower",
  ...
  expect_equal(
    nrow(summary_timeseries),
    length(models) * length(exp_scores) * nb_timeseries
  )

  expect_named(summary_timeseries, c(
    "timeseries", "score", "model", "bottom",
  ...
  expect_named(summary_variables, c(
    "timeseries", "horizon", "score", "model",
...
test_that("Summary are handled properly when there is just one model or no field 'model'", {
  ...
  expect_named(summary_no_model, c(
    "score", "bottom", "lower", "median", "mean",
    "upper", "top", "sd"
  ))
})
...
test_that("Summary works for outputs of 'evaluate_model'", {
  ...
  out <- evaluate_model(sampled_obs, sampled_cases,
    model = a_model,
    horizon = 7, samples = 10,
    serial_interval = EpiSoon::example_serial_interval
  )
